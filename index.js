 ...
 * currently Vincent Van Dough. Use an array method to fix this issue and
 ...
 * getArtistByIndex returns a string in the format `The artist at index {id}
 * is {name}.`
 *
 * For example, if getArtistByIndex is invoked with the inventory and the number
 * 0, it will return `The artist at index 0 is Amedeo Modigliani`.
 */
...
*/
function removeArtist(arr, arrIndex) { arr.splice(arrIndex, 1); }
...
argument and returns an array with names of artists who painted more than 100
paintings
*/
...
function addArtist(info) { artists.push(info); }
...
addArtist({
  ...
  bio :
      "Stay at home mother of a 6 year old, who enjoys playing videogames and watching drama. One of her pet peeves is that she hates people touching her food (unless necessary)"
});
...
/* STRETCH 1: Create a function called get20s() that takes data as an argument
 * and returns an array with names of artists who were born the 20th century
 * (1800-1900) */

function get20s(/* Code here */) { /* Code here */ }
...
<div class="image">
    <img
src="https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg"/>
</div>
<div class = "name">
   <a href="https://en.wikipedia.org/wiki/Vincent_van_Gogh"> Vincent Van
Gogh</a>
</div>
...
art. In just over a decade he created about 2,100 artworks, including around 860
oil paintings, most of them in the last two years of his life. They include
landscapes, still lifes, portraits and self-portraits, and are characterised by
bold colours and dramatic, impulsive and expressive brushwork that contributed
to the foundations of modern art. However, he was not commercially successful,
and his suicide at 37 followed years of mental illness and poverty.</div>
</div>
...
‼️ You do **NOT** need to get these to display on your page, but you can copy
and paste the result into your HTML file if you'd like to see what that would
look like. */

function getHTML(/* Code here */) { /* Code here */ }

/* STRETCH 3: Create a function called `randomize` that takes a data array as an
 * argument and returns a the same array in a randomized order. */

function randomize(/* Code here */) { /* Code here */ }

/* STRETCH 3: Use advanced array methods (.map, .reduce, .filer) to refactor
 * your MVP code (create an array of all artists born in the 1900s with .filter,
 * for example) */
